...
export function computeMetrics(
  ...
  for (let year = 1; year <= inputs.horizonYears; year++) {
    ...
    const totalOwner = yearPoints.reduce(
      (sum, p) => sum + p.ownerUnrecoverableMonthly,
      0
    );
  ...
  // First check if owner starts ahead (at month 1)
  if (timeline.length > 0) {
    const firstPoint = timeline[0];
    if (firstPoint.ownerNetWorth >= firstPoint.renterNetWorth) {
      metrics.netWorthBreakEvenYear = 1; // Owner is already ahead from the start
    } else {
      // Check monthly for when owner catches up (more accurate than year-end only)
      for (let month = 1; month <= timeline.length; month++) {
        const point = timeline[month - 1];
        if (point.ownerNetWorth >= point.renterNetWorth) {
          // Calculate which year this month falls into
          metrics.netWorthBreakEvenYear = Math.ceil(month / 12);
          break;
        }
      }
    }
